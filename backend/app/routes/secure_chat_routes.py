...
from app.core.strict_mode import strict_production_enabled
...
async def register_secure_chat_endpoints(app, sessions_dict: Dict):
    ...
    async def unified_chat(request: Request):
        ...
        body.get("response_type", "auto").lower()

        # ===== Input Validation =====
        if not query or len(query) < 2:
            raise HTTPException(
                status_code=400, detail="Query required (minimum 2 characters)"
            )
        ...
        is_suspicious, injection_confidence, reason = (
            PromptInjectionDetector.is_suspicious(query)
        )
        ...
        try:
            ...
            if df is None or df.empty:
                raise HTTPException(
                    status_code=400, detail="Dataset is empty or invalid"
                )
        ...
        try:
            ...
            # ===== Confidence Check =====
            if result["confidence"] < UnifiedChatEngine.MIN_CONFIDENCE and result.get(
                "fallback"
            ):
                result["warning"] = (
                    f"Low confidence response ({result['confidence']:.0%}). May be inaccurate."
                )
            ...
            if strict_production_enabled():
                raise HTTPException(
                    status_code=503,
                    detail="Unified chat unavailable in strict production mode without real inference response.",
                )
    ...
    async def stream_chat(request: Request):
        ...
        async def generate():
            ...
            result = UnifiedChatEngine.process_query(
                ...
                analytics=sessions_dict[dataset_id]
                .get("pipeline", {})
                .get("analytics", {}),
                ml_results=sessions_dict[dataset_id]
                .get("pipeline", {})
                .get("ml_predictions", {}),
