...
class FinancialGreeks:
    ...
    def _validate_inputs(
        ...
        checks = {
            ...
            "volatility_in_range": FinancialGreeks.VOLATILITY_MIN <= volatility <= FinancialGreeks.VOLATILITY_MAX,
            "rate_in_range": FinancialGreeks.RISK_FREE_RATE_MIN <= risk_free_rate <= FinancialGreeks.RISK_FREE_RATE_MAX,
        }
    ...
    def calculate_greeks(
        ...
        vega = (
            spot_price * np.exp(-dividend_yield * time_to_expiry) * norm.pdf(d1) *
            np.sqrt(time_to_expiry) / 100
        )
        
        if option_type == OptionType.CALL:
            theta = (
                -spot_price * np.exp(-dividend_yield * time_to_expiry) * norm.pdf(d1) * volatility / (2 * np.sqrt(time_to_expiry)) -
        ...
        rho = (
            strike_price * time_to_expiry * np.exp(-risk_free_rate * time_to_expiry) *
            (norm.cdf(d2) if option_type == OptionType.CALL else -norm.cdf(-d2)) / 100
        )
    ...
    def implied_volatility(
        ...
        for iteration in range(100):
            ...
            result = FinancialGreeks.calculate_greeks(
                ...
                vol, risk_free_rate, option_type
            )
