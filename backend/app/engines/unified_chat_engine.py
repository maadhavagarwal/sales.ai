...
import numpy as np
import pandas as pd
from app.core.strict_mode import strict_production_enabled, require_real_services
...
class UnifiedChatEngine:
    ...
    def _handle_chart_request(
        query: str, df: pd.DataFrame, base_confidence: float
    ) -> Dict:
    ...
    def _handle_analysis_request(
        query: str, df: pd.DataFrame, analytics: Dict, ml_results: Dict, pipeline: Dict
    ) -> Dict:
        ...
        try:
            ...
            if not answer or len(answer) < 10:
                answer = UnifiedChatEngine._generate_fallback_answer(
                    query, df, analytics
                )
                confidence = 0.72
            else:
    ...
    def _handle_qa_request(
        query: str, df: pd.DataFrame, analytics: Dict, ml_results: Dict
    ) -> Dict:
    ...
    def _generate_fallback_answer(query: str, df: pd.DataFrame, analytics: Dict) -> str:
        ...
        require_real_services("Unified chat fallback answer")
