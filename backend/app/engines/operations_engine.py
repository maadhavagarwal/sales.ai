...
class OperationsEngine:
    ...
    def get_operations_data():
        ...
        try:
            # 1. Personnel
            personnel = [
                dict(row) for row in conn.execute("SELECT * FROM personnel").fetchall()
            ]

            # 2. Tasks
            tasks = [
                dict(row) for row in conn.execute("SELECT * FROM tasks").fetchall()
            ]
            ...
            schedules = [
                ...
                for row in conn.execute(
                    "SELECT * FROM operational_schedules"
                ).fetchall()
            ]

            return {"personnel": personnel, "tasks": tasks, "schedules": schedules}
    ...
    def manage_schedule(action: str, data: dict):
        ...
        try:
            if action == "CREATE":
                ...
                conn.execute(
                    "DELETE FROM operational_schedules WHERE id=?", (data.get("id"),)
                )
                conn.commit()
                return {"status": "DELETED"}
        finally:
