...
def generate_analytics(df):
    ...
    if revenue_col:
        ...
        analytics["average_order_value"] = (
            float(df[revenue_col].mean()) if not df[revenue_col].empty else 0
        )
    ...
    for col in df.columns:
        if str(col).lower() not in standard_cols and pd.api.types.is_numeric_dtype(
            df[col]
        ):
    ...
    if variant_cols:
        ...
        analytics["variant_performance"] = {
            str(k): float(v) for k, v in variant_sums.items()
        }

        # If there's no explicit product column, these variants ARE the products
        if not product_col:
            analytics["top_products_from_variants"] = {
                str(k): float(v) for k, v in variant_sums.head(10).items()
            }
    ...
    if (
        ...
        try:
            ...
            analytics["monthly_revenue_trend"] = {
            ...
            analytics["daily_revenue_trend"] = {
                str(k): float(v) for k, v in daily_trend.items()
            }
        except Exception:
            pass
