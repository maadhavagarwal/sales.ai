...
def build_dataset_index(df):
    ...
    text_cols = [c for c in cols if df[c].dtype == 'object' or 'name' in c.lower() or 'id' in c.lower()]
    date_cols = [c for c in cols if 'date' in c.lower() or 'time' in c.lower()]
    metric_cols = [c for c in cols if c not in text_cols and c not in date_cols and pd.api.types.is_numeric_dtype(df[c])]
    ...
    for idx, row in df.iterrows():
        ...
        for c in text_cols[:4]:
            if pd.notna(row[c]): id_parts.append(f"{c} is '{row[c]}'")
        if id_parts: parts.append(". ".join(id_parts))
        
        vals = [f"{c}={row[c]}" for c in metric_cols if pd.notna(row[c])]
        if vals: parts.append("Statistical values: " + ", ".join(vals))
...
def search_dataset(query, k=5):
    ...
    try:
        ...
        distances, indices = vector_stored_data["index"].search(np.array(query_vector).astype('float32'), 20)
