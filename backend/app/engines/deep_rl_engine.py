...
class PricingEnvironment:
    ...
    def step(self, action_idx):
        ...
        next_state = np.array(
            [self.current_price / self.base_price, self.step_count / float(self.max_steps)],
            dtype=np.float32,
        )
...
def _train_dqn_torch(env, episodes):
    ...
    return {
        ...
        "neural_intelligence": (
            f"DQN Agent converged on {final_adj}% adjustment after {episodes} training cycles. "
            "High sensitivity to profit-velocity crossover detected."
        ),
    }
...
def _train_dqn_simple(env, episodes):
    ...
    return {
        ...
        "confidence": 0.65,
        "neural_intelligence": (
            "Heuristic agent identified price equilibrium point. "
            "Install 'torch' to activate high-fidelity Deep Q-Learning workstation."
        ),
    }
