    ...
    try:
        if extension.lower() in [".xlsx", ".xls"]:
            ...
            df_preview = pd.read_excel(
                file_io, header=None, nrows=30, sheet_name=sheet_name
            )
        else:
            file_io.seek(0)
            try:
                ...
                df_preview = pd.read_csv(
                    file_io, header=None, nrows=30, encoding="latin1"
                )
        ...
        for i, row in df_preview.iterrows():
            matches = sum(
                1 for cell in row if any(p in str(cell).lower() for p in patterns)
            )
...
def load_data_robustly(file_io, filename, sheet_name=None):
    ...
    if len(df) > 0:
        for i in range(1, min(5, len(df))):
            last_row = df.iloc[-i]
            row_str = " ".join(
                str(val).lower() for val in last_row.values if pd.notnull(val)
            )
            if "total" in row_str and sum(pd.notnull(last_row.values)) < (
                len(df.columns) / 2
            ):
                df = df.iloc[:-i]
                break
            elif "total" in row_str and any(
                kw in row_str for kw in ["all", "grand", "sum"]
            ):
                df = df.iloc[:-i]
                break
