...
def run_pipeline(df):
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            ...
            if dataset_type in ["sales_dataset", "market_dataset"]:
                ...
                if dataset_type == "sales_dataset":
                    ...
                    price_col = (
                        ...
                        else ("price" if "price" in df.columns else "revenue")
                    )
                    future_sims = executor.submit(
                        MarketDynamicsEngine.calculate_indicators,
                        df,
                        price_col=price_col,
                    )
            ...
            if "revenue" in df.columns:
                target_col = next(
                    (
                        c
                        for c in ["product", "group", "category", "region", "customer"]
                        if c in df.columns
                    ),
                    None,
                )
                if target_col:
                    future_clusters = executor.submit(
                        run_clustering, df, target_col, "revenue"
                    )
            ...
            try:
                if future_clusters:
                    ...
                    if clusters:
                        insights.append(
                            f"🧠 AI identified {len(clusters)} optimal performance tiers."
                        )
            except:
                pass
            ...
            try:
                ...
                if forecast_res and "forecast" in forecast_res:
                    ...
                    if forecast_list:
                        ...
                        insights.append(
                            f"🔮 Yield prediction: ₹{last_pred:,.2f} projected peak node."
                        )
            except:
                pass
    ...
    if "quantity" in df.columns:
        ...
        variant_cols = [
            c
            for c in df.columns
            if str(c).isnumeric() or str(c).replace(".", "", 1).isnumeric()
        ]
        if variant_cols:
    ...
    if "revenue" not in df.columns:
        ...
        if "revenue" in df.columns:
            ...
            analytics["average_revenue"] = (
                float(df["revenue"].mean()) if not df["revenue"].empty else 0
            )
    ...
    if "total_profit" in analytics and analytics.get("total_revenue", 0) > 0:
        analytics["average_margin"] = (
            analytics["total_profit"] / analytics["total_revenue"]
        ) * 100
    ...
    if "training_results" in ml_results and isinstance(
        ml_results["training_results"], dict
    ):
    ...
    confidence_score = (
        (data_quality * 0.4)
        + (ml_confidence * 0.5)
        + (0.1 if not drift_detected else 0)
    )
    ...
    try:
        strategic_plan = generate_detailed_strategic_plan(
            analytics, insights, strategy, pricing_opt
        )
    except:
    ...
    return {
        ...
        "market_intelligence": {
            ...
            "indicators": (
                market_data.get("indicators", pd.DataFrame()).to_dict(orient="records")
