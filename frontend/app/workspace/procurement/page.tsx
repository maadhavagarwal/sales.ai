    ...
    <div className="p-8 space-y-8 max-w-7xl mx-auto">
      <header className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 mt-1">Manage purchase orders and inter-location stock transfers</p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList className="bg-slate-900/50 p-1 border border-slate-800">
          <TabsTrigger value="procurement" className="data-[state=active]:bg-emerald-600 data-[state=active]:text-white">
            <ShoppingCart className="w-4 h-4 mr-2" /> Purchase Orders
          </TabsTrigger>
          <TabsTrigger value="logistics" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
             <Card className="glass-pro bg-slate-900/20 border-slate-800">
                <CardContent className="pt-6">
                    <p className="text-xs text-slate-500 uppercase tracking-widest font-bold">Planned Spend (90d)</p>
                    <h3 className="text-2xl font-bold mt-1 text-white">₹1.2Cr</h3>
                </CardContent>
             </Card>
             <Card className="glass-pro bg-slate-900/20 border-slate-800">
                <CardContent className="pt-6">
                    <p className="text-xs text-slate-500 uppercase tracking-widest font-bold">Open POs</p>
                    <h3 className="text-2xl font-bold mt-1 text-orange-400">12 Pending</h3>
                </CardContent>
             </Card>
             <Card className="glass-pro bg-slate-900/20 border-slate-800">
                <CardContent className="pt-6">
                    <p className="text-xs text-slate-500 uppercase tracking-widest font-bold">Supplier Lead Time</p>
                    <h3 className="text-2xl font-bold mt-1 text-emerald-400">4.2 Days</h3>
                </CardContent>
             </Card>
             <Card className="glass-pro bg-slate-900/20 border-slate-800">
                <CardContent className="pt-6">
                    <p className="text-xs text-slate-500 uppercase tracking-widest font-bold">Reorder Alerts</p>
          ...
          <Card className="glass-pro bg-slate-900/20 border-slate-800">
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>Track status of external stock procurement</CardDescription>
              </div>
              <div className="relative w-64">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3 h-3 text-slate-500" />
                <Input placeholder="Filter POs..." className="pl-8 h-8 text-xs glass-pro border-slate-800" />
              </div>
            </CardHeader>
            <CardContent>
                ...
                <tbody className="divide-y divide-slate-800/50">
                  {[
                    ...
                    <tr key={po.id} className="group hover:bg-white/5 transition-colors">
                      ...
                      <td className="py-4 px-2 font-semibold">{po.supplier}</td>
                      <td className="py-4 px-2 font-bold">{po.value}</td>
                      <td className="py-4 px-2 text-slate-400 text-xs">{po.date}</td>
                      ...
                      <td className="py-4 text-right">
                        <Button variant="ghost" size="sm" className="h-7 text-xs text-slate-400 hover:text-white">View</Button>
                      </td>
                    </tr>
                  ))}
                </tbody>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <Card className="glass-pro bg-slate-900/40 border-slate-800">
                <CardHeader>
                    ...
                    <CardDescription>Move stock between warehouses or retail outlets</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <label className="text-[10px] uppercase text-slate-500 font-bold ml-1">Source Node</label>
                        ...
                        <div className="space-y-1">
                            <label className="text-[10px] uppercase text-slate-500 font-bold ml-1">Destination Node</label>
                            <select className="w-full bg-slate-950 border border-slate-800 rounded-md p-2 text-sm">
                    ...
                    <div className="space-y-1">
                        <label className="text-[10px] uppercase text-slate-500 font-bold ml-1">Select SKU</label>
                        <Input placeholder="Enter SKU ID or Product Name..." className="bg-slate-950 border-slate-800 h-10" />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <label className="text-[10px] uppercase text-slate-500 font-bold ml-1">Quantity</label>
                            <Input type="number" defaultValue={1} className="bg-slate-950 border-slate-800 h-10" />
                        </div>
                        <div className="space-y-1 flex items-end">
                             <Button className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold h-10">
                                Authorize Transfer
                             </Button>
                        </div>
                    </div>
              ...
              <div className="space-y-4">
                 ...
                 <div className="space-y-2">
                    {[
                        ...
                        <div key={node.name} className="p-3 rounded-lg bg-slate-900/40 border border-slate-800/50 flex justify-between items-center">
                            <div>
                                <p className="font-semibold text-sm">{node.name}</p>
                                <p className={`text-[10px] font-bold ${node.color} uppercase tracking-tight`}>{node.status}</p>
                            </div>
                            <div className="text-right">
                                <p className="text-lg font-bold font-mono">{node.cap}</p>
                            </div>
                        </div>
                    ))}
                 </div>
           ...
           <Card className="glass-pro bg-slate-900/20 border-slate-800">
             <CardHeader className="flex flex-row items-center justify-between">
               <CardTitle className="text-lg flex items-center gap-2">
                   <History className="w-4 h-4 text-slate-400" /> Logistic Feed
               </CardTitle>
               <Button variant="ghost" size="sm" className="text-xs text-slate-500">Full Audit Log</Button>
             </CardHeader>
             <CardContent>
               <div className="space-y-3">
                 {[1, 2, 3].map(i => (
                   <div key={i} className="flex justify-between items-center p-3 text-xs border-b border-slate-800/30">
                     <div className="flex gap-3 items-center">
                        <div className="p-1.5 rounded-full bg-emerald-500/10 text-emerald-500">
                            <CheckCircle className="w-3 h-3" />
                        </div>
                        <div>
                            <span className="font-semibold">MUM → DEL</span>
                            <p className="text-slate-500 mt-0.5">50x RTX 4090 TI (SKU: G-8821) Transferred</p>
                        </div>
                     </div>
