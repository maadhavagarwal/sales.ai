...
export default function WorkspaceNexus() {
    ...
    const [uploadResults, setUploadResults] = useState<FileStatus[]>([])
    ...
    const handleBulkUpload = async (files: FileList) => {
        ...
        try {
            ...
            if (result.status === "SUCCESS") {
                setUploadResults(result.analysis.map((r: any) => ({
                    name: r.name,
                    category: r.category,
                    records: r.records,
                    status: r.status
    ...
    return (
        <div className="space-y-8 pb-20">
            ...
            <div className="relative h-[300px] w-full flex items-center justify-center overflow-hidden rounded-3xl border border-white/10 bg-black/40 backdrop-blur-xl mb-12">
                ...
                <div className="relative z-10 text-center">
                    <motion.div
                        ...
                        className="w-24 h-24 rounded-full bg-gradient-to-tr from-[--primary] to-[--accent-cyan] flex items-center justify-center shadow-[0_0_50px_rgba(99,102,241,0.5)] mx-auto mb-6"
                    >
                        <span className="text-3xl">🧩</span>
                    </motion.div>
                    <h2 className="text-2xl font-black uppercase tracking-[0.2em] text-white">Data Nexus</h2>
                ...
                <AnimatePresence>
                    {isUploading && uploadResults.map((file, idx) => (
                        <motion.div
                            ...
                            className="absolute p-3 rounded-xl bg-white/5 border border-white/10 backdrop-blur-md"
                        >
                            <p className="text-[8px] font-black text-white truncate max-w-[80px]">{file.name}</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <Card variant="glass" className="lg:col-span-2 p-8 relative overflow-hidden group">
                    ...
                    <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-4">
                        {uploadResults.map((res, i) => (
                            ...
                            >
                                <p className="text-[9px] font-black text-white truncate">{res.name}</p>
                                <Badge 
                                    size="xs" 
                ...
                <Card variant="glass" className="p-8">
                    <h3 className="text-sm font-black uppercase tracking-widest text-white mb-6">Silo Persistance</h3>
                        ...
                        {[
                            ...
                            <div key={i} className="space-y-2">
                                <div className="flex justify-between items-center">
                                    <span className="text-[10px] font-black uppercase tracking-widest text-white/60">{silo.label}</span>
                                    <span className="text-[10px] font-black text-white">{silo.count.toLocaleString()}</span>
                                </div>
                    ...
                    <div className="mt-10 p-4 rounded-2xl bg-[--primary]/10 border border-[--primary]/20">
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <p className="text-[9px] font-black text-white uppercase tracking-widest leading-normal">Data Encryption Active</p>
