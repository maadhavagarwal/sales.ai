...
export default function LiquidityForecast({ data }: { data: LiquidityData }) {
    ...
    return (
        <Card variant="glass" padding="lg" className={`overflow-hidden relative ${!isHealthy ? 'border-amber-500/20' : 'border-[--accent-emerald]/20'}`}>
            <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-white/10 to-transparent rotate-45 translate-x-16 -translate-y-16" />
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-10 relative z-10">
                ...
                <div className="text-right">
                    <p className="text-[10px] font-black text-white/40 uppercase tracking-[0.3em] mb-2">Projected 90D Balance</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10 relative z-10">
                <div className="p-6 rounded-2xl bg-black/40 border border-white/5">
                    <p className="text-[9px] font-black text-white/30 uppercase tracking-[0.3em] mb-4">Current Reserves</p>
                    <p className="text-xl font-black text-white tracking-tight">{formatCurrency(data.current_cash)}</p>
                ...
                <div className="p-6 rounded-2xl bg-black/40 border border-white/5">
                    ...
                    <p className="text-xl font-black text-white tracking-tight">{formatCurrency(data.receivables_pipeline)}</p>
                    <div className="mt-4 h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                        <div className="h-full bg-[--accent-emerald]" style={{ width: '85%' }} />
                    </div>
                    <p className="text-[8px] font-black text-[--accent-emerald]/60 uppercase tracking-[0.1em] mt-2">85% Collection Reliability</p>
                </div>

                <div className="p-6 rounded-2xl bg-black/40 border border-white/5">
                    <p className="text-[9px] font-black text-red-400 uppercase tracking-[0.3em] mb-4">Projected Outflow</p>
                    <p className="text-xl font-black text-white tracking-tight">{formatCurrency(data.payables_pipeline)}</p>
                    <div className="mt-4 h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
            ...
            <div className="space-y-4 relative z-10">
                <div className="flex items-center gap-4 py-4 px-6 rounded-2xl bg-white/[0.02] border border-white/5">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-xs font-bold text-white/80">{data.recommendation || "Maintain current collection velocity."}</p>
                    </div>
                </div>
            </div>
        </Card>
